...
export function LoginForm() {
  ...
  // Client-side fallback check to avoid showing login form if session is already active
  React.useEffect(() => {
    let isMounted = true;
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Set sb-remember preference cookie prior to signing in
      if (keepSignedIn) {
        document.cookie = `sb-remember=true; path=/; max-age=${60 * 60 * 24 * 365}; SameSite=Lax`;
      } else {
        // Session cookie without max-age or expires (cleared on browser close)
              ...
              <div className="flex items-center justify-between py-1">
                ...
                >
                  <input
                    ...
                    className="h-4 w-4 rounded border-border text-primary focus:ring-primary/20 bg-surface cursor-pointer accent-indigo-500 transition-all"
                  />
